...
class UARTDeviceState(UARTGenericState):
    """
    UART Finite State Machine Device State.
    This state is connected with UART Modem Device state.
    """
    ...
    def on_enter(fsm_instance):
        ...
        :return:                None
        """
        fsm_instance.event_mgr.uart_state_changed(ModemState.Device)
    ...
    def init_node_event_message_event(fsm_instance, msg):
        """
        Enter Init Node state and update registered models
        Change state to Unknown.
    ...
    def current_state_response_message_event(fsm_instance, msg):
        ...
        :return:                None
        """
        if msg.state != ModemState.Device:
            fsm_instance.event_mgr.uart_unexpected_message(msg.type)
            fsm_instance.change_state(UART_FSMState.Unknown)
